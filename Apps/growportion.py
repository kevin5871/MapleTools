...
def charset() :
    ...
    if optionvar[0] != str(-1) and socket.gethostbyname(socket.gethostname()) != '127.0.0.1' and useDefChar.get() == True :
...
def draw(level, percent) :
    ...
    if level != -1 :
        x = round(400 * percent/100)
        canvas.itemconfig(ChangeInfo, text="%s레벨 / %s%%"%(level,percent))
        canvas.coords(ChangeBar, 0,220,x,230)
...
button_3 = Button(image=button_image_3, borderwidth=0, highlightthickness=0, command= lambda : close())
...
ChangeBar = canvas.create_rectangle(0,220,0,230, fill="#00FF00", outline="")
